"""Standalone version of Structured State Space sequence model (S4). From https://github.com/state-spaces/s4. """
...
class S4(nn.Module):
    ...
    def __init__(self, model_size, num_layers=1, **kwargs):
        super().__init__()
        self.ssm_blocks = nn.ModuleList([
            S4Block(model_size, transposed=False, **kwargs)
            for _ in range(num_layers)
        ])

    def forward(self, x: torch.Tensor, lengths=None, **kwargs):
        # x : L, B, N
        x = x.permute(1, 0, 2)  # B, L, N
        for block in self.ssm_blocks:
            x, _ = block(x, lengths=lengths, **kwargs)
        x = x.permute(1, 0, 2)
        return x
